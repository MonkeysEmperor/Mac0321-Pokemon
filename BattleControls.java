...
public class BattleControls extends Controller {
	...
	BattleControls (){
		...
		t[0].setactivePokemon(t[0].p[0]);
		t[1].setactivePokemon(t[1].p[0]);
	...
	private class Fight extends Event {
		...
		public void action() {
			...
			if(t[y].activePokemon.hp <= 0){
				...
				t[y].pokemon -= 1;
				System.out.println(t[y].activePokemon.getName() + " has fainted");
				t[y].setactivePokemon(t[y].p[j++]);
			}
		}
	...
	private class Pack extends Event {
		...
		public String description() {
			if(potion == true) return t[x].getName() + " used a Potion";
			else return t[x].getName() + " doesn't have any potions";
		}
	}
	...
	private class Switch extends Event {
		...
		public void action() {
			if(i < 5){
			   t[x].setactivePokemon(t[x].p[i++]);
			}
			else t[x].setactivePokemon(t[x].p[0]);
	...
	public static void main(String[] args) {
		...
		long tm = System.currentTimeMillis();
		bc.addEvent(bc.new StartBattle(tm));
		bc.addEvent(bc.new Round(tm + 1000));
		bc.run();
	}
} 
